...
costheta_bins = np.arange(-1.0, 1.0, 0.025)
sintheta_bins = np.arange(-1.0, 1.0, 0.025)
theta_dot_bins = np.arange(-8.0, 8.0, 0.05)
action_bins = np.arange(-2.0, 2.0, 0.01)
    ...
    while True:
        ...
        for t in range(1000):
            ...
            if np.random.random() < epsilon or previous_observation is None:
                action_val = env.action_space.sample()  # random action
            else:
