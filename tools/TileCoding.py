...
def estimate_observation_space_range(env):
    state_ranges = []
    if np.inf in env.observation_space.high or np.inf in env.observation_space.low:
        env.render()
        env.reset()
        observations = []
        ...
        for col in range(0, np.shape(np.asarray(observations))[1]):
            ...
            state_ranges.append([ob_min, ob_max])
